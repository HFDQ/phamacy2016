...
namespace BugsBox.Pharmacy.AppClient.UI.Forms.SalesBusiness
{
    public partial class FormSalesOrderEdit : BaseFunctionForm
    {
        ...
        public FormSalesOrderEdit(SalesOrder salesOrder)
        {
            ...
            var listsod = salesOrder.SalesOrderDetails.Where(r => r.Deleted == false && r.Amount > 0).ToList();
        ...
        private void btnDetailAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("请选择收货公司的信息！！！", "信息", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            catch (Exception ex)
            {
        ...
        private void cmbPurchase_DropDown(object sender, EventArgs e)
        {
            ...
            {
                Log.Error(ex);
                MessageBox.Show("打开客户选择框失败！！！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void InitControl()
        {
            ...
            if (_salesOrder != null)
            {
                this.Text = this._salesOrder.OrderStatusValue >= (int)OrderStatus.Approved ? "配送出库单记录" : "配送出库单编辑";
            ...
            else
            {
                ...
                listSD = listSD.Where(r => r.Deleted == false && r.Amount > 0).ToList();
                ...
                else
                {
                    ...
                    {
                        bool IsChinese = _drugInventoryRecordlList.Where(r => r.DrugInfo.BusinessScopeCode.Contains("中药材") || r.DrugInfo.BusinessScopeCode.Contains("中药饮片")).Count() > 0;
                        _salesDrugType = IsChinese ? SalesDrugType.ChineseDrug : SalesDrugType.Drug;
                        SetGridColumnVisible();
                        toolStripButton4.Visible = IsChinese;
                        tsbtnPrint.Visible = !IsChinese;
                    }
                }
        ...
        public void submit()
        {
            ...
            try
            {
                if (MessageBox.Show("确定要提交吗？", "提示", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
                {
                    ...
                    else
                    {
                        ...
                        this.PharmacyDatabaseService.WriteLog(AppClientContext.CurrentUser.Id, "成功新建配送出库记录成功：" + _salesOrder.OrderCode);
                        this.Close();
                    }
            ...
            {
                MessageBox.Show("配送出库订单提交操作失败！！！" + ex.Message, "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                this.PharmacyDatabaseService.WriteLog(AppClientContext.CurrentUser.Id, "成功新建配送出库记录失败");
                Log.Error(ex);
            }
        }
                    ...
                    if (MessageBox.Show("确定要审核提交吗？", "提示", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
                    {
                        try
                        {
                            ...
                            else
                            {
                                if (f.DialogResult == System.Windows.Forms.DialogResult.OK)
                                {
                                    if (this.SalePriceControlRuleModel.SalesOrderPrintRuleValue == EnumSalesOrderPrintRule.销售单审核后打印)
                                    {
                                        if (MessageBox.Show("需要打印单据吗？", "提示", MessageBoxButtons.OKCancel) == System.Windows.Forms.DialogResult.OK)
                                        {
                                            ...
                                            if (_drugInventoryRecordlList.Where(r => r.DrugInfo.BusinessScopeCode.Contains("中药")).Count() > 0)
                                            {
                                ...
                                this.Close();
                                this.PharmacyDatabaseService.WriteLog(AppClientContext.CurrentUser.Id, "执行配送出库审批记录成功");
                            }
                        }
                        catch (Exception ex)
                        {
        ...
        private void tsbtnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (MessageBox.Show("确定要保存吗？", "提示", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
                {
                    ...
                    {
                        this.PharmacyDatabaseService.WriteLog(AppClientContext.CurrentUser.Id, "成功更新一条配送出库记录成功：" + _salesOrder.OrderCode);
                        this.Close();
                    }
            ...
            {
                MessageBox.Show("配送出库订单保存操作失败！！！" + msg, "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Log.Error(ex);
            }
        }
        ...
        private void tsbtnPrint_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (_salesOrder != null)
                {
                    ...
                    string FirstChecker = nodelist[0].Attributes["FirstChecker"].Value.ToString();
                    string SecondChecker = nodelist[0].Attributes["SecondChecker"].Value.ToString();
                    string Checker = FirstChecker + " " + SecondChecker;
                    ds.ExtendedProperties.Add("Checker", Checker);
                    string InventoryKeeper = nodelist[0].Attributes["inventoryKeeper"].Value.ToString();
                    ...
                    ds.ExtendedProperties.Add("TransportMethod", manufacturer);
                    ...
                    this.PharmacyDatabaseService.WriteLog(AppClientContext.CurrentUser.Id, "成功打印配送出库记录：" + _salesOrder.OrderCode);
                }
                else
                {
        ...
        private void toolStripButton6_Click(object sender, EventArgs e)
        {
            ...
            MyExcelUtls.DataGridview2Sheet(this.dgvDrugDetailList, "配送出库单" + this._salesOrder.OrderCode);
        }
