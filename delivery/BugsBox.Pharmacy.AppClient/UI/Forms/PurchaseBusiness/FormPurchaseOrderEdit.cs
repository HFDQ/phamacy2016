    ...
    public partial class FormPurchaseOrderEdit : BaseFunctionForm
    {
        ...
        private void btnCancel_Click(object sender, EventArgs e)
        {
            ...
            if (form.DialogResult == DialogResult.OK)
            {
                ...
                try
                {
                    ...
                    this.PharmacyDatabaseService.WriteLog(AppClientContext.CurrentUser.Id, "执行配送入库单取消操作成功,单号:"+purchaseOrder.DocumentNumber);
                }
                catch
                {
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show("您至少需要增加一条药品信息,请选择药品", "错误", MessageBoxButtons.OK);
                    this.btnSubmit.Enabled = true;
                    return;
                }
                foreach (DataGridViewRow dr in this.dataGridView1.Rows)
                {
                    if (Convert.ToDecimal(dr.Cells[clmDrugNumber.Name].Value) <= 0 )
                    {
                        MessageBox.Show("数量为0");
                        this.btnSubmit.Enabled = true;
                ...
                MessageBox.Show("配送入库记录修改成功", "提示", MessageBoxButtons.OK);

                this.PharmacyDatabaseService.WriteLog(AppClientContext.CurrentUser.Id, "执行更新配送入库记录单细节操作成功，配送入库单号：" + updateOrder.DocumentNumber);
            }
            catch (Exception ex)
            {
        ...
        private void btnModifyPurchaseAmount_Click(object sender, EventArgs e)
        {
            ...
            {
                this.dataGridView1.EndEdit();
                if (MessageBox.Show("确定已经修改了正确的配送入库数量吗？", "提示", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
                {
                    string msg = string.Empty;
                    #region 构造主表
                    PurchaseOrder order = new PurchaseOrder();
                    ...
                    MessageBox.Show("配送入库单数量修改申请成功", "提示", MessageBoxButtons.OK);
                    this.PharmacyDatabaseService.WriteLog(BugsBox.Pharmacy.AppClient.Common.AppClientContext.currentUser.Id, "成功提交配送入库单修改申请");
                }
            }
            catch (Exception ex)
            {
        ...
        private void toolStripButton1_Click_1(object sender, EventArgs e)
        {
            ...
            MyExcelUtls.DataGridview2Sheet(this.dataGridView1, "配送入库订单"+s);
        }
