    ...
    public partial class FormReceivingOrder : BaseFunctionForm
    {
        ...
        private int validInputInDataGridView()
        {
            ...
            if (b.Count() > 0)
            {
                ...
                var re = MessageBox.Show("品名：(" + names + ")，到货数量超出配送入库单数量，请修改！");
                return 0;
            }
            ...
            if (d.Count() > 0)
            {
                ...
                {
                    var re = MessageBox.Show("品名：(" + names + ")，到货数量不足，需要执行修改配送入库数量流程吗？", "提示", MessageBoxButtons.OKCancel);
        ...
        private void tsbtnAccept_Click_1(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (String.IsNullOrEmpty(msg))
                {
                    ...
                    {
                        MessageBox.Show("配送入库数量与到货数量不同，需经过审批修改配送入库数量");
                    }
                    else
                    {
                        MessageBox.Show("收货完成");
                        this.PharmacyDatabaseService.WriteLog(AppClientContext.CurrentUser.Id, "执行配送入库单收货操作成功,采购单号：" + _order.RelatedOrderDocumentNumber);
                        this.Close();
                    }
                }
                else
                {
                    ...
                    this.PharmacyDatabaseService.WriteLog(AppClientContext.CurrentUser.Id, "执行配送入库单收货操作失败,采购单号：" + _order.RelatedOrderDocumentNumber);
                }
            }
            catch
            {
        ...
        private void toolStripButton2_Click(object sender, EventArgs e)
        {
            ...
            MyExcelUtls.DataGridview2Sheet(this.dataGridView1, "配送入库收货单");
        }
    }
}
