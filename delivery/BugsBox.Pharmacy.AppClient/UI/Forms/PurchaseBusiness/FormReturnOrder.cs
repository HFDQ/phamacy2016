...
namespace BugsBox.Pharmacy.AppClient.UI.Forms.PurchaseBusiness
{
    ...
    public partial class FormReturnOrder : BaseFunctionForm
    {
        ...
        private void tsbtnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("确定要提交配送入库退货流程吗？", "提示", MessageBoxButtons.OKCancel,MessageBoxIcon.Warning) != System.Windows.Forms.DialogResult.OK) return;
            try
            {
                if (validInputInDataGridView())
                {
                    ...
                    string orderNumber = this.PharmacyDatabaseService.CreatePurchaseOrderReturnByEnity(out msg, _order, _orderDetails.ToArray());
                    if (String.IsNullOrEmpty(msg))
                    {
                        lblOrderStatus.Text = EnumHelper<OrderReturnStatus>.GetDisplayValue((OrderReturnStatus)_order.OrderStatus);
                        lblOrderNo.Text = orderNumber;
                        MessageBox.Show("退货申请完成");
                        this.PharmacyDatabaseService.WriteLog(AppClientContext.CurrentUser.Id, "执行配送入库退货申请操作成功,单号：" + orderNumber);
        ...
        {
            if (MessageBox.Show("确定要取消该配送入库退货单据吗？", "提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) != System.Windows.Forms.DialogResult.OK) return;
            string message = "";
            try
            {
                this.PharmacyDatabaseService.CancelPurchaseReturnOrder(this._order.Id, out message);
                this.PharmacyDatabaseService.WriteLog(AppClientContext.CurrentUser.Id, "执行配送入库退货取消操作成功");
                this.DialogResult = System.Windows.Forms.DialogResult.OK;
                this.Dispose();
            }
            catch (Exception ex)
        ...
        {
            if (MessageBox.Show("确定要提交配送入库退货出库拣货流程吗？", "提示", MessageBoxButtons.OKCancel) != System.Windows.Forms.DialogResult.OK) return;
            ...
            {
                this.PharmacyDatabaseService.WriteLog(AppClientContext.CurrentUser.Id, "执行配送入库退货单通知出库操作成功,单号：" + por.DocumentNumber);
                MessageBox.Show("提交配送入库退货出库拣货流程成功！");
                this.DialogResult = System.Windows.Forms.DialogResult.OK;
            ...
            {
                MessageBox.Show("提交配送入库退货出库拣货流程失败！");
            }
        }
        ...
        private void toolStripButton2_Click(object sender, EventArgs e)
        {
            ...
            if (Check == null)
            {
                MessageBox.Show("该配送入库退货单暂未复核，请复核后再打印"); return;
            }
            ...
            this.PharmacyDatabaseService.WriteLog(BugsBox.Pharmacy.AppClient.Common.AppClientContext.currentUser.Id, "成功打印配送入库退货单，单号：" + _order.DocumentNumber);
        }
        ...
        private void toolStripButton4_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    this.PharmacyDatabaseService.WriteLog(AppClientContext.CurrentUser.Id, "执行配送入库退货单出库质量复核操作成功,单号：" + por.DocumentNumber);
                    MessageBox.Show("配送单成功生成，请至配送申请中查询该退单信息！");
                    this.DialogResult = System.Windows.Forms.DialogResult.OK;

                    if (MessageBox.Show("需要打印该配送入库退货单吗？", "提示", MessageBoxButtons.OKCancel) == System.Windows.Forms.DialogResult.OK)
                    {
                        this.toolStripButton2_Click(sender, e);
                    }
                ...
                {
                    this.PharmacyDatabaseService.WriteLog(AppClientContext.CurrentUser.Id, "执行配送入库退货单出库质量复核操作失败,单号：" + por.DocumentNumber);
        ...
        {
            if (MessageBox.Show("确认拣货，并将该退单提交至配送入库退货出库质量复查流程吗？", "提示", MessageBoxButtons.OKCancel) != System.Windows.Forms.DialogResult.OK) return;

            PurchaseOrderReturn por = this.PharmacyDatabaseService.GetPurchaseOrderReturn(out msg, _order.Id);
            por.OrderStatus = OrderReturnStatus.ReturnPickupChecked;
            if (this.PharmacyDatabaseService.SavePurchaseOrderReturn(out msg, por))
            {
                MessageBox.Show("提交配送入库退货出库质量复查流程成功！");
                this.PharmacyDatabaseService.WriteLog(AppClientContext.CurrentUser.Id, "执行配送入库退货单出库拣货操作成功,单号：" + por.DocumentNumber);
                this.DialogResult = System.Windows.Forms.DialogResult.OK;
                this.Dispose();
            }
            else
            {
                this.PharmacyDatabaseService.WriteLog(AppClientContext.CurrentUser.Id, "执行配送入库退货单出库拣货操作失败,单号：" + por.DocumentNumber);
                MessageBox.Show("提交配送入库退货出库质量复查流程失败！");
