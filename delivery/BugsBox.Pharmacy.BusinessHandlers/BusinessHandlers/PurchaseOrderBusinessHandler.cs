...
namespace BugsBox.Pharmacy.BusinessHandlers
{
    partial class PurchaseOrderBusinessHandler
    {
        ...
        public string CreatePurchaseOrder(PurchaseOrder order, List<PurchaseOrderDetail> orderDetails)
        {
            try
            {
                lock (LockObj)
                {
                    ...
                    bool isUpdate = false;
                    PurchaseOrder existOrder = BusinessHandlerFactory.RepositoryProvider.Db.PurchaseOrders.FirstOrDefault(p => p.Id == order.Id);
                    if (existOrder == null)
                    {
                        ...
                        order.PurchasedDate = order.PurchasedDate;
                    ...
                    DbSet<PurchaseOrderDetail> dbSet = BusinessHandlerFactory.RepositoryProvider.Db.Set<PurchaseOrderDetail>();
                    if (isUpdate)
                    {
                        foreach (PurchaseOrderDetail d in orderDetails)
                        {
                            PurchaseOrderDetail detail = BusinessHandlerFactory.RepositoryProvider.Db.PurchaseOrderDetails.FirstOrDefault(p => p.Id == d.Id);
                            if (detail != null)
                            {
                                detail.UpdateTime = DateTime.Now;
                                detail.UpdateUserId = order.UpdateUserId;
                                detail.Amount = d.Amount;
                                detail.AmountOfTax = d.AmountOfTax;
                                detail.PurchasePrice = d.PurchasePrice;
                                detail.sequence = d.sequence;
                                detail.Deleted = d.Deleted;
                                if (d.Deleted)
                                {
                            ...
                            else
                            {
                                ...
                                d.StoreId = order.StoreId;
                                d.UpdateTime = DateTime.Now;
                                d.UpdateUserId = order.CreateUserId;
                                d.PurchaseOrderId = order.Id;
                                dbSet.Add(d);
                            }
                        }
                        order.TotalMoney = orderDetails.Where(r => r.Deleted == false).Sum(r => r.Amount * r.PurchasePrice);
                    }
                    else
                    {
                        int i = 0;
                        ...
                        foreach (PurchaseOrderDetail d in orderDetails)
                        {
                            ...
                            if (i == 0)
                            {
                                SupplyUnitSalesman salesMan = BusinessHandlerFactory.RepositoryProvider.Db.SupplyUnitSalesmans.FirstOrDefault(p => p.SupplyUnitId == d.PurchaseOrderId);
                                if (salesMan != null)
                                {
                                    order.SupplyUnitAccountExecutiveId = salesMan.Id;
                                }
                            }
                            i++;
        ...
        public string CreatePurchaseInInventeryOrderByEnity(PurchaseCommonEntity order, List<PurchaseInInventeryOrderDetailEntity> orderDetails)
        {
            try
            {
                PurchaseInInventeryOrder pio = RepositoryProvider.Db.PurchaseInInventeryOrders.Where(r => r.PurchaseOrderId == order.PurchaseOrderId).FirstOrDefault();
