...
namespace BugsBox.Pharmacy.BusinessHandlers
{
    partial class OutInventoryBusinessHandler
    {
        ...
        /// <param name="q"></param>
        /// <returns></returns>
        public DrugOutInventoryCheckModel[] GetDrugOutInventoryChecksByQueryModel(Business.Models.DrugOutInventoryCheckQueryModel q)
        {
            var all = RepositoryProvider.Db.OutInventoryDetails.Where(r => r.CreateTime > q.DTF && r.CreateTime < q.DTT);
            ...
            if (!string.IsNullOrEmpty(q.BatchNumber))
            {
            ...
            {
                all = all.Where(r => r.FactoryName.Contains(q.FactoryName));
            }

            var re = from i in all
                     join sd in RepositoryProvider.Db.SalesOrderDetails on i.SalesOrderDetailId equals sd.Id
                     join di in RepositoryProvider.Db.DrugInventoryRecords on sd.DrugInventoryRecordID equals di.Id
                     join u1 in RepositoryProvider.Db.Users on i.SalesOutInventory.OrderOutInventoryCheckUserID equals u1.Id
                     join u2 in RepositoryProvider.Db.Users on i.SalesOutInventory.SpecialDrugSecondChecker equals u2.Id
                     into left
                     from le in left.DefaultIfEmpty()
                     select new Business.Models.DrugOutInventoryCheckModel
                     {
                         FactoryName = i.FactoryName,
                         BatchName = i.BatchNumber,
                         ...
                         PurchaseUnitName = sd.SalesOrder.PurchaseUnit.Name,
                         SaleDate = sd.SalesOrder.SaleDate,
                         SalesOrderCode = sd.SalesOrder.OrderCode,
                         SpecificName = i.SpecificationCode,
                         IsSpecial = di.DrugInfo.IsSpecialDrugCategory,
                         FirstCheckerName = u1.Employee.Name,
                         SecondCheckerName = le == null ? "无" : le.Employee.Name,
