    ...
    partial class SalesOrderBusinessHandler
    {
        ...
        public void AddSalesOrderAndDetails(SalesOrder so)
        {
            ...
            try
            {
                lock (LockObj)
                {
                    ...
                    so.OrderOutInventoryUserID = this.GetSalePriceControlRules().InventoryKeeperId;
            ...
            try
            {
                ...
                if (input.StatisticObject == (int)StatisticObject.Drug)
                {
                    var c = from i in this.Queryable
                            ...
                            join m in RepositoryProvider.Db.PurchaseInInventeryOrderDetails on di.PurchaseInInventeryOrderDetailId equals m.Id
                            select new
                            {
                                ...
                                saleNum = sd.Amount,
                                returnSaleNum = 0m,
                                supplier = m.PurchaseInInventeryOrder.PurchaseOrder.SupplyUnit.Name
                            };
                    var cc = from i in RepositoryProvider.Db.SalesOrderReturns
                             where i.OrderReturnStatusValue != 9 && i.OrderReturnCheckTime >= input.FromDate && i.OrderReturnCheckTime <= input.ToDate
                    ...
                    result = c.ToList().Concat(cc.ToList()).GroupBy(p => p.drugid)
                                .Select(p => new SalesOrderStatisticOutput
                                {
                                    ...
                                    CanSaleNum = p.Sum(o => o.CanSaleNum),
                                    Count = p.Count(),
                                    Sum = p.Sum(r => r.InvoiceMoney),
                                    CostSum = p.Sum(r => r.unitMoney),
        ...
        public System.Collections.Generic.IEnumerable<Business.Models.SaleOrderModel> GetSalesOrderCodePaged(Business.Models.SalesCodeSearchInput searchInput)
        {
            try
            {
                ...
                var outInv = from p in RepositoryProvider.Db.OutInventorys
                             ...
                             join u4 in usr on p.storekeeperId equals u4.Id
        ...
        public List<SalesOrderRecordOutput> GetSalesOrderRecordPaged(SalesOrderRecordInput searchInput, out PagerInfo pager, int pageindex, int pageSize)
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(searchInput.Seller))
                {
                    saleoders = saleoders.Where(r => r.SalerName.Contains(searchInput.Seller));
                }
                ...
                var result = from so_p in so_ps
                             ...
                             select new SalesOrderRecordOutput()
                             {
                                 ...
                                 MeasurementUnit = drug_dir_sod.sod.MeasurementUnit,
                                 ...
                                 DictionaryMeasurmentUnitCode = drug_dir_sod.drug.DictionaryMeasurementUnitCode
                             };
                //result.ToList();
        ...
        public void CancelSalesOrder(SalesOrder so)
        {
            try
            {
                var origin = this.RepositoryProvider.Db.SalesOrders.FirstOrDefault(r => r.Id == so.Id);
                if (origin.OrderStatus == OrderStatus.Canceled) return;
        ...
        public IEnumerable<Business.Models.SaleOrderModel> GetSaleRefundHistory(SalesCodeSearchInput searchInput)
        {
            ...
            var result = from i in varOrderCodeBalance
                         ...
                         join n in RepositoryProvider.Db.Users on i.CreateUserId equals n.Id
                         join o in RepositoryProvider.Db.Employees on n.EmployeeId equals o.Id
                         orderby i.BalanceTime descending
                         select new Business.Models.SaleOrderModel
                         {
                             BalanceTime = i.BalanceTime,
                             BalanceUserName = l.Name,
        ...
        public System.Collections.Generic.IEnumerable<Business.Models.SalesOrderModelForSalesOrderReturn> GetSalesOrderByOrderModel(Business.Models.SalesOrderQueryModel m)
        {
            ...
            if (!string.IsNullOrEmpty(m.PurchaseUnitKeyword))
            {
                ...
                c = c.Where(r => r.PurchaseUnit.PinyinCode != null).Where(r => r.PurchaseUnit.PinyinCode.ToUpper().Contains(m.PinyinCode) || r.PurchaseUnit.Name.Contains(m.PurchaseUnitKeyword));
            }
            if (!string.IsNullOrEmpty(m.PinyinCode))
            {
                c = c.Where(r => r.PurchaseUnit.PinyinCode != null).Where(r => r.PurchaseUnit.PinyinCode.ToUpper().Contains(m.PinyinCode));
            }
        ...
        /// <returns></returns>
        public System.Collections.Generic.IEnumerable<Business.Models.Model_IdName> GetInventoryKeepers(string keyword)
        {
            var allusers = base.RepositoryProvider.Db.Users.Where(r => r.Deleted == false && r.Enabled);

            var roles = RepositoryProvider.Db.Roles.Where(r => r.Name.Contains("保管员"));
        ...
        /// <param name="q"></param>
        /// <returns></returns>
        public Business.Models.SalesOrderForVATModel[] GetVATModelsbyQueryModel(Business.Models.SalesOrderForVATQueryModel q)
        {
            var c = this.RepositoryProvider.Db.SalesOrders.Where(r => r.Deleted == false && r.OrderStatusValue >= (int)OrderStatus.Banlaced);
            if (!string.IsNullOrEmpty(q.Keyword))
            {
                c = c.Where(r => r.OrderCode.Contains(q.Keyword));
            }

            if (!string.IsNullOrWhiteSpace(q.PurchaseUnitName))
            {
                c = c.Where(r => r.PurchaseUnit.Name.Contains(q.PurchaseUnitName) || r.PurchaseUnit.PinyinCode.Contains(q.PurchaseUnitName));
            }

            var re = from i in c
                     join store in RepositoryProvider.Db.Stores on i.StoreId equals store.Id
                     select new Business.Models.SalesOrderForVATModel
                     {
                         Id = i.Id,
        ...
        /// <returns></returns>
        public bool SaveVATCode(Guid Id, string VATCode, string VATNumber, decimal VATRate, string Bank)
        {
            var m = this.Get(Id);
            m.VATCode = VATCode;
            m.VATNumber = VATNumber;
            m.VATRate = VATRate;
            var pu = this.BusinessHandlerFactory.PurchaseUnitBusinessHandler.Get(m.PurchaseUnitId);
            pu.Bank = Bank;
            this.Save();
            return true;
        }
        ...
        /// <summary>
        /// 根据销售单ID获取VAT劳务清单的细节信息
        /// </summary>
        /// <param name="SalesOrderId">销售单ID</param>
        /// <returns>Business.Models.SalesOrderDetailForVATModel列表</returns>
        public Business.Models.SalesOrderDetailForVATModel[] GetSalesOrderDetailForVATModels(System.Guid SalesOrderId)
        {
            var a = this.Get(SalesOrderId);
            var purchaseUnit = this.RepositoryProvider.PurchaseUnitRepository.Get(a.PurchaseUnitId);
            ...
            var re = from i in a.SalesOrderDetails.OrderBy(r => r.Index)//依据开单顺序排序 
                     ...
                     {
                         Amount = i.Amount,
                         MeasurementName = i.MeasurementUnit,
                         ...
                         AddAndTel = addressAndTel,
                         BankAccount = bankAccount,
                         TaxNumber = BusinessLicense == null ? "" : BusinessLicense.LicenseCode,
                         Memo = a.Description
                     };
            return re.ToArray();
        }
        ...
        /// <returns></returns>
        public bool SaveVATForSalesOrderDetails(Business.Models.SalesOrderDetailForVATModel[] list)
        {
            foreach (var i in list)
            {
                var m = this.RepositoryProvider.SalesOrderDetailRepository.Get(i.SalesOrderDetailId);
                m.Discount = i.Discount;
            }
            this.Save();
            return true;
        }
