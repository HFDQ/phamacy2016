...
namespace BugsBox.Pharmacy.AppClient.UI.Forms.SalesBusiness
{
    public partial class FormSalesOrderIndex : BaseFunctionForm
    {
        ...
        private void FormSalesOrderIndex_Load(object sender, EventArgs e)
        {
            ...
            cmbSeller.DisplayMember = "py";
            cmbSeller.ValueMember = "name";
            cmbSeller.DataSource = c;
            cmbSeller.AutoCompleteSource = AutoCompleteSource.ListItems;
            cmbSeller.AutoCompleteMode = AutoCompleteMode.SuggestAppend;
        ...
        private void FormatRows()
        {
            foreach (DataGridViewRow row in this.dgvMain.Rows)
            {
                ...
                if (entity != null)
                {
                    try
                    {
                        row.Cells["保管员"].Value = BugsBoxApplication.Instance.InventoryKeeper;
                    }
                    catch (Exception ex)
                    {
        ...
        {
            MyExcelUtls.DataGridview2Sheet(this.dgvMain, "销售定单列表");
        }
        ...
        {
            string name = this.cmbSeller.SelectedValue.ToString();
