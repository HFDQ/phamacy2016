...
namespace BugsBox.Pharmacy.AppClient.UI.Forms.PurchaseBusiness
{
    public partial class FormPurchaseOrderGenerator : BaseFunctionForm
    {
        ...
        private Business.Models.SalePriceControlRulesModel CurrentSalePriceControlModel;
        ...
        public FormPurchaseOrderGenerator()
        {
            ...
            label4.Text = "";
            lable7.Text = DateTime.Now.ToString("yyyy年MM月dd日 HH时mm分");
            ...
            this.CurrentSalePriceControlModel = this.PharmacyDatabaseService.GetSalePriceControlRules(out msg);
                    ...
                    for (int i = 0; i < listSupply.Count; i++)
                    {
                        ...
                        for (int j = 0; j < this.dataGridView1.Rows.Count; j++)
                        {
                            if (listSupply[i].Equals(this.dataGridView1.Rows[j].Cells["clmSupplyUnitId"].Value.ToString()))
                            {
                                ...
                                {
                                    if (MessageBox.Show("该行价格为0，如果是赠品（赠品价格允许为0，但不允许为负数），则点击确定；如不是，则请点击取消修改", "提示", MessageBoxButtons.OKCancel) == System.Windows.Forms.DialogResult.Cancel) return;
            ...
            try
            {
                foreach (var unit in listDrugInfo)
                {
                    ...
                    if (!exist)
                    {
                        ...
                        this.dataGridView1.Rows[index].Cells["AmountOfTax"].Value = this.CurrentSalePriceControlModel.PurchaseOrderDefaultTaxRate.DefaultTaxRate;
        ...
        private bool GetSupplyList()
        {
            try
            {
                ...
                for (int i = 0; i < this.dataGridView1.Rows.Count; i++)
                {
                    ...
                    decimal drugPurchasePrice = Decimal.Parse(this.dataGridView1.Rows[i].Cells["clmPurchasePrice"].EditedFormattedValue.ToString());
                    if (drugAmount <= 0)
                    {
                    ...
                    if (drugPurchasePrice <= 0)
                    {
                        this.dataGridView1.Rows[i].Cells["clmPurchasePrice"].Style.BackColor = CellDataErrorBackColor;
                        MessageBox.Show("采购价格不能小于零元", "提示", MessageBoxButtons.OK);
                        return false;
                    }
                    else
                    {
                        this.dataGridView1.Rows[i].Cells["clmPurchasePrice"].Style.BackColor = CellDataValidBackColor;
                    }
                }
                return true;
            }
        ...
        private void toolStripButton5_Click(object sender, EventArgs e)
        {
            ...
            frm.GoodsType = this.GoodsType;//采购商品类型控制
            frm.Show(this);
