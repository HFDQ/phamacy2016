...
namespace BugsBox.Pharmacy.AppClient.UI.Forms.PurchaseBusiness
{
    public partial class FormPurchaseOrderApproval : BaseFunctionForm
    {
        ...
        {
            this.rtbDescription.Text = ((ListItem)(cmbApprovalStatus.SelectedItem)).Name+"：采购单据编号:"+this._purchaseOrder.DocumentNumber;
        }
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (_orderStatus == OrderStatus.Approved)
                    {
                        ...
                        this.PharmacyDatabaseService.WriteLog(AppClientContext.CurrentUser.Id, "执行采购单审核操作成功，单号：" + _purchaseOrder.DocumentNumber);
                    }
                    else
                    {
                        ...
                        this.PharmacyDatabaseService.WriteLog(AppClientContext.CurrentUser.Id, "执行采购单数量修改审核操作成功，单号：" + _purchaseOrder.DocumentNumber);
                    }
