...
namespace BugsBox.Pharmacy.AppClient.UI.Forms.PurchaseBusiness
{
    ...
    public partial class FormCheckOrder : BaseFunctionForm
    {
        ...
        int SupplyDrugType = 0;
        bool IsChineseDrugNeedOutValidDate = false;//中药材或饮片是否需要填写有效期至？
            ...
            : this()
        {
            ...
            foreach (DataGridViewRow r in dataGridView1.Rows)
            {
                ...
                if (bs.Contains("中药材") || bs.Contains("中药饮片"))
                {
                    if (bs.Contains("中药材"))
                    {
                        this.dataGridView1.Columns[this.colOutValidDate.Name].Visible = this.IsChineseDrugNeedOutValidDate;
        ...
        public void BindDataInfoes()
        {
            ...
            foreach (DataGridViewRow r in dataGridView1.Rows)
            {
                ...
                if (this.isChineseDrug)
                {
                    r.Cells[this.colOutValidDate.Name].Value = DateTime.Now.AddYears(3).Year.ToString() + DateTime.Now.Month.ToString().PadLeft(2, '0') + DateTime.Now.Day.ToString().PadLeft(2, '0'); ;

                    if (this.IsChineseDrugNeedOutValidDate)
                    {
                        r.Cells[this.colPruductDate.Name].Value = DateTime.Now.Year.ToString() + DateTime.Now.Month.ToString().PadLeft(2, '0') + DateTime.Now.Day.ToString().PadLeft(2, '0');
                    }
                ...
                {                   
                    foreach (DataGridViewRow row in rows)
                    {
                        ...
                        if (cellOutValidDate.Value == null)
                        {
                            if (this.IsChineseDrugNeedOutValidDate)
                            {
                                MessageBox.Show("中药材、中药饮片同样需要填写有效期至，请填写完整八位码效期，如：20140101");
                            }
                            else
                            {
                                MessageBox.Show("非中药材、中药饮片需要填写有效期至，请填写完整八位码效期，如：20140101");
                            }
                            return false;
                        }
